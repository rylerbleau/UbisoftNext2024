...
#define INPUT_DEADZONE  ( 0.50f * FLOAT(0x7FFF) )  // Default to 10% of the +/- 32767 range.   Changed to 50% because my controller is finicky.
//-----------------------------------------------------------------------------
...
void CSimpleControllers::Update()
{
	...
	if (numControllers == 0 )
	{
		// i dont want to have false controllers, so just return here
		return;
